...
#    theprobs = comb(g,g-1)*((1-Pfr_es)**(g-1))*Pfr_es  #equation 8 from ESTCP paper for g-g'=1
    theprobs = g*((1-Pfr_es)**(g-1))*Pfr_es
    ...
    return(theresult)
@njit
def findcolp_es(usl, bat_charge, navail_es, es_energy, es_powercapacity, rt_efficiency, Pfr_es):
            ...
            for t in range(168):
                if t > 0:
                    if usl[i1, i2, t] > 0:  #if unserved load >0, that is, gens NOT ENOUGH to feed load!
                        if (navail_es[i1,i2] > 0) and (es_powercapacity  > usl[i1,i2,t ]/max(navail_es[i1,i2],1) ):
                        ...
                        if (navail_es[i1,i2] > 0 and bat_charge[i1, i2, t-1]< es_energy ):   #charge battery IF it isn't full
                            charging_current = min(es_powercapacity, -usl[i1,i2,t ]*rt_efficiency/navail_es[i1,i2])
                            bat_charge[i1,i2,t]=bat_charge[i1,i2,t-1]+charging_current
                            bat_charge[i1,i2,t]=min(bat_charge[i1,i2,t], es_energy) #cant continue charging once it's full
...
def failfindbat(usl, navail_es, es_energy, es_powercapacity, rt_efficiency, Pfr_es):
    arrayshape=usl.shape
    # matrix same size as usl but fill with zeros to start
    niters=arrayshape[0]
#    timestart1 = time.time()
    bat_charge = np.zeros((niters,8760, 168)) #track battery charge
    bat_charge[:,:,0]=es_energy #assume initial battery charge - is FULL
    failcasearr1=findcolp_es(usl, bat_charge, navail_es, es_energy, es_powercapacity, rt_efficiency, Pfr_es)
 #   timestop1=time.time()
