...
This version DOES subtract PV power production from the gross critical load. Purpose is to compare to the "variable load" reliability
curves provided by ESTCP, show difference PV array makes. NO BATTERY case.
...
def loopthrut(usl, navail, Pfr, cload, gensize):    #not used in this code
    ...
    return(usl)
def readin(file1,  niters, Pfs, mtbf, scalefactor, ngendiff, nofuel):
 #   Pfs = 0.002
 #   mtbf=1700
 #   niters=40  # # of iterations in MCS
    Pfr= 1- np.exp(-1/mtbf)
    if nofuel == 1:  #for no fuel case
        genkill=0
    else:
        genkill = 1
...
#    ab=read_excel('NASCorpusChristi.xlsx', sheet_name='Critical Loads & Distribution')
    ...
    ngens=ngens1[0]+ ngendiff   #change the # of generators compared to ESTCP base case!!!
    criticalfrac=crf[0]
    cload = scalefactor*gross_output * criticalfrac - pv_power #the critical load NET of PV power produced
 ...
 #       print('time step ', t, '\n')
        ...
        usl[:,:,t] = cload_shift[:,t] - gensize*navail*genkill
    data_out={'failcase': failcasearr, 'usl': usl}
    return(data_out)
